...
SATSUMA_EXPORT MatchingResult solve_matching(Matching const &mp,
                              MatchingSolver solver = MatchingSolver::Default);

SATSUMA_EXPORT MatchingResult solve_matching_via_lemon(Matching const &mp);

/// The matching problem must be feasible, otherwise Blossom-V warns of undefined behaviour!
SATSUMA_EXPORT MatchingResult solve_matching_via_blossomV(Matching const &mp);
